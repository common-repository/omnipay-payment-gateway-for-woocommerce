<?php
	...
	class WC_omnipay_Hosted extends WC_Payment_Gateway {
		...
		public  $type 			= 'hosted';
		private $gateway_frontend_url_prod = 'https://gateway.omni-pay.com';
		private $gateway_backend_url_prod = 'https://api.omni-pay.com/gateway/api/v1';
		...
		public function generate_omnipay_hosted_form($order_id) {
			...
			$gateway_frontend_url = $this->gateway_frontend_url_prod;
			...
			$response = $this->OmniPayApi([
				'payload' =>$payload,
				'endpoint' => "/internal/session/create"
			]);
		...
		function process_response() {
			...
			$responseSessionID = sanitize_text_field($_POST['sessionID']);
			...
			$payload_sale_notification = [
				"session_id" => $responseSessionID,
				"order_notes" => $orderNotes,
				"wp_order_status_before" => $order->status,
				"wp_payment_complete_status" => "0"
			];
			
			//getting cancel url by default
			$redirect_url = $order->get_cancel_order_url( $order );

			if ($responseCode === '0') {
				$order->set_transaction_id($responsetransactionUnique);
				$payload_sale_notification["wp_transaction_id_status"] = $order->get_transaction_id();
				$payload_sale_notification["wp_add_order_note_status"] = $order->add_order_note( __(ucwords( $this->gateway ).' payment completed.' . $orderNotes, 'woocommerce_omnipay') );
				$payload_sale_notification["wp_payment_complete_status"] = $order->payment_complete();
				$payload_sale_notification["wp_order_status_after"] = $order->status;
				
				//overwriting success url
				$redirect_url = $this->get_return_url( $order );
				
			} 

			$payload_sale_notification["wp_redirect_url"] = $redirect_url;
			
			$this->OmniPayApi([
				'payload' =>$payload_sale_notification,
		...
		function OmnipayApi($params){
			
			try{
				$payload = $params['payload'];
				$endpoint = $params['endpoint'];
					
				$gateway_backend_url = $this->gateway_backend_url_prod;
				if($this->settings['production_mode'] == 'custom'){
					$gateway_backend_url = $this->settings['apiUrl'];
				}
				
				$accessKey = sanitize_text_field($this->settings['accessKey']);
				$signature = $this->createSignature($payload, sanitize_text_field($this->settings['secretKey']));
				...
				return wp_remote_post( $gateway_backend_url.$endpoint, $args );
			}catch(Exception $e){
